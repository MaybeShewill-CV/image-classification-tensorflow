...
class CNNBaseModel(metaclass=ABCMeta):
    ...
    def separate_conv(self, input_tensor, output_channels, kernel_size, name, depth_multiplier=1,
    ...
    @staticmethod
    def globalavgpooling(inputdata, data_format='NHWC', name=None, keepdims=False):
        """
        ...
        return tf.reduce_mean(input_tensor=inputdata, axis=axis, name=name, keep_dims=keepdims)
    ...
    def pad(inputdata, paddings, name):
        ...
        """
        with tf.variable_scope(name_or_scope=name):
            return tf.pad(tensor=inputdata, paddings=paddings, name='pad_output')
